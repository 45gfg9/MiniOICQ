...
WebSocketThread::WebSocketThread(QObject *parent) : QThread(parent), _socket(new QWebSocket) {
    connect(_socket, &QWebSocket::connected, this, &WebSocketThread::onConnected);
    connect(_socket, &QWebSocket::disconnected, this, &WebSocketThread::onDisconnected);
    connect(_socket, &QWebSocket::binaryMessageReceived, this, &WebSocketThread::onMessageReceived);
}
...
void WebSocketThread::disconnectSocket() {
    _socket->close();
}
...
void WebSocketThread::onMessageReceived(const QByteArray &message) {
    qDebug() << "Received message from [" << _wsUrl << "]: " << message.length();
    emit messageReceived(message);
}

void WebSocketThread::messageReceived(const QByteArray &message) {
    // TODO
}
...
void WebSocketThread::on_login(const QString& userId, const QString& password);
{
    ...
    // connect(loginProxyModel, &LoginProxyModel::login, this, &WebSocketThread::login);
    // Send login message
    // if success
